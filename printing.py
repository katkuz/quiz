...
def print_question(current_question, questions_and_answers, current_question_index):
    ...
    if correct_percent < 0:  # Print without correct percentage cause we can not calculate it
        print(
            f"Fråga #{current_question + 1} [id: {questions_and_answers[current_question_index]['id']}]:"
            f" {questions_and_answers[current_question_index]['prompt']}")
    else:
        print(
            f"Fråga #{current_question + 1} [id: {questions_and_answers[current_question_index]['id']}; "
            f"{correct_percent}% har svarat rätt]: {questions_and_answers[current_question_index]['prompt']}")
...
def correct_answer(answer_time, current_point, total_points):
    ...
    print(
        f"Rätt! Du svarade på {answer_time} sekunder och får {current_point} poäng!\nDu har {total_points} poäng totalt!\n")
...
def final(questions_and_answers, wrong_answered_questions, total_correct_answers, total_points):
    """ Print final info: total points and all incorrect answered questions with correct answers """
    print(f"\nGrattis!\nDu har svarat korrekt på {total_correct_answers} frågor! Du har fått {total_points} poäng!")
